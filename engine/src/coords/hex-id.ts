...
export function getColIdxFromColId(colId: string): number {
  if (!/^[A-Z]$/.test(colId)) {
...
export function getColIdFromColIdx(colIdx: number): string {
...
export function getRowIdxFromRowId(rowId: string): number {
...
export function getRowIdFromRowIdx(rowIdx: number): string {
...
export function getPartsFromHexId(id: string): { col: string; row: number } {
...
export function getAxialFromHexId(id: string): AxialHex {
  const match = /^([A-Z])-([1-9][0-9]*)$/.exec(id);
  if (!match) {
    throw new Error(`Invalid hex id: ${JSON.stringify(id)}. Expected format 'A-1'.`);
  }
  const q = getColIdxFromColId(match[1]!);
...
export function getHexIdFromAxial(hex: AxialHex): string {
  const { q, r } = hex;
  const col = getColIdFromColIdx(q);
  const row = 2 * r + q + 1;
  return `${col}-${row}`;
}
